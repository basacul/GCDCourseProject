...
names(test) <- c("Subject", "Train" , features , "Activity")
...
names(train) <- c("Subject", "Train" , features , "Activity")
...
good <-     grepl("mean", colnames(dataframe), ignore.case = TRUE) |
            grepl("std", colnames(dataframe), ignore.case = TRUE)

subset <- dataframe[,good]
subset <- cbind(dataframe[,c(1,2,ncol(dataframe))],subset)
...
l <- nrow(subset)
subset[,3] <- as.character(subset[,3])
for (i in 1:l){
      
      if( subset[i,3] == "1"){
            subset[i,3] <- "walking"
      }else if(subset[i,3] == "2"){
            subset[i,3] <- "walking_upstairs"
      }else if(subset[i,3] == "3"){
            subset[i,3] <- "walking_downstairs"
      }else if(subset[i,3] == "4"){
            subset[i,3] <- "sitting"
      }else if(subset[i,3] == "5"){
            subset[i,3] <- "standing"
      }else if(subset[i,3] == "6"){
            subset[i,3] <- "laying"
      }else{
            subset[i,3] <- NA
      }
...
#?done?  For me it looks ok

#5. From the data set in step 4, creates a second, independent tidy data set with 
#the average of each variable for each activity  and each subject.
library(dplyr)
#Remove Train Column
subset <- select(subset, -Train)

subset_Activity <- select(subset, -Subject)
subset_Subject <- select(subset, -Activity)

activity <- group_by(subset, Activity)
subject <- group_by(subset, Subject)

active <- summarize(activity, mean)
subjective < su
